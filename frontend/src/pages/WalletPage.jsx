...
//                             <Box bg={cardBg} p={{ base: 4, md: 6 }} borderRadius={RV.radius.md} border="1px solid" borderColor={RV.slate[200]}>
...
const WalletPage = ({ url }) => {
  ...
  const selectView = (view) => {
    setActiveView(view);
    if (view === 'investments') setSearchParams({}, { replace: true });
  ...
  const fetchWalletBalance = useCallback(async () => {
    try {
      ...
      if (!res.ok) throw new Error('Could not load wallet data');
      const data = await res.json();
      setDigitalMoney(Number(data.digital_money) || 0);
      setHasPendingWithdrawal(data.hasPendingWithdrawal || false);
    } catch (err) {
      toast({ title: 'Error', description: err.message, status: 'error', duration: 3000 });
    }
  ...
  const confirmCancelWithdrawal = useCallback(async () => {
    ...
    try {
      ...
      showToast(toast, {
        ...
        description: 'The request has been cancelled and the amount is back in your wallet.',
        status: 'success',
      });
  ...
  const handleAcceptOffer = useCallback(async (selectedPrice, stockCount) => {
    ...
    try {
      ...
      if (data.has_bonus && data.bonus_amount > 0) {
        ...
        showToast(toast, { title: 'Sold', description: 'The trade has been booked.', status: 'success' });
      }
  ...
  return (
    <>
      <VendorShell
        ...
        url={url}
        tabs={[
          { id: 'investments', label: 'Investments', active: activeView === 'investments', onSelect: () => selectView('investments') },
          { id: 'history', label: 'History', active: activeView === 'history', onSelect: () => selectView('history') },
          { id: 'claims', label: 'Claims', active: activeView === 'claims', onSelect: () => selectView('claims') },
          { id: 'earnings', label: 'Earnings', active: activeView === 'earnings', onSelect: () => selectView('earnings') },
          { id: 'deposits', label: 'Deposits', active: activeView === 'deposits', onSelect: () => selectView('deposits') },
          { id: 'withdrawals', label: 'Withdrawals', active: activeView === 'withdrawals', onSelect: () => selectView('withdrawals') },
        ]}
      >
          <Container maxW="100%" p={0}>
            {error && <Alert status="error" mb={4} borderRadius={RV.radius.md} bg={RV.errorBg} borderColor={RV.errorBorder} color={RV.error}><AlertIcon color={RV.error} />{error}</Alert>}
            ...
            {activeView === 'investments' && (
              <Box bg="var(--panel)" p={{ base: 4, md: 6 }} borderRadius="var(--r-structure)" border="1px solid" borderColor="var(--border)">
                <Heading size="md" mb={4}>Active investments</Heading>
                {activeTrades === null ? (
                  <SkeletonRows count={4} />
                ) : activeTrades.length === 0 ? (
                  <EmptyState headline="No active investments" body="Bought stock will appear here while it is held." actionLabel="Buy stock" actionTo="/vendor/products" />
                ) : (
                  <VStack spacing={4} align="stretch">
            ...
            {activeView === 'history' && (
              <Box bg="var(--panel)" p={{ base: 4, md: 6 }} borderRadius="var(--r-structure)" border="1px solid" borderColor="var(--border)">
                <Heading size="md" mb={4}>Sold trades</Heading>
                {soldTrades === null ? (
                  <SkeletonRows count={4} />
                ) : soldTrades.length === 0 ? (
                  <EmptyState headline="No sold trades yet" body="Once you sell an unlocked holding it will show here." />
                ) : (
                  <VStack spacing={4} align="stretch">
                    {soldTrades.map((item) => (
                      <SoldTradeItem 
                     ...
                     {soldTrades.length > 0 && (
                       <Box
                         ...
                         textAlign="center"
                       >
                         <Text fontSize="lg" fontWeight="bold" color={totalProfit >= 0 ? 'var(--ok)' : 'var(--bad)'}>
                           Total profit from sold trades
                         </Text>
                         {totalProfitLoading ? (
                           <Center p={4}>
            ...
            {activeView === 'deposits' && (
              <Box bg="var(--panel)" p={{ base: 4, md: 6 }} borderRadius="var(--r-structure)" border="1px solid" borderColor="var(--border)">
                <Heading size="md" mb={8}>Deposit history</Heading>
                {depositHistory === null ? (
                  <SkeletonRows count={4} />
                ) : depositHistory.length === 0 ? (
                  <EmptyState headline="No deposits yet" body="Add money to see deposits listed here." actionLabel="Add money" onAction={() => setAddMoneyOpen(true)} />
                ) : (
                  <>
                    <TransactionList title="Approved" type="deposit" transactions={depositHistory.filter(tx => tx.status === 'approved')} headingColor="green.400" />
                    <TransactionList title="Pending" type="deposit" transactions={depositHistory.filter(tx => tx.status === 'pending')} headingColor="yellow.400" />
            ...
            {activeView === 'withdrawals' && (
              <Box bg="var(--panel)" p={{ base: 4, md: 6 }} borderRadius="var(--r-structure)" border="1px solid" borderColor="var(--border)">
                <Heading size="md" mb={8}>Withdrawal history</Heading>
                {withdrawalHistory === null ? (
                  <SkeletonRows count={4} />
                ) : withdrawalHistory.length === 0 ? (
                  <EmptyState headline="No withdrawals yet" body="Request a withdrawal to send funds to your bank." />
                ) : (
                  <>
                    <TransactionList title="Approved" type="withdrawal" transactions={withdrawalHistory.filter(tx => tx.status === 'approved')} headingColor="green.400" />
                    <TransactionList 
      ...
      <ConfirmationModal
        ...
        message={`Cancel your withdrawal request of ${formatMoney(confirmationModal.amount)}?`}
